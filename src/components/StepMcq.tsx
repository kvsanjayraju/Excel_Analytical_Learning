    ...
    <div className="space-y-6">
      ...
      {submitted && (
        <motion.div
          ...
          className={clsx("p-4 rounded-lg space-y-4", isCorrect ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800")}
        >
          <div>
          ...
          {isCorrect ? (
            <button
                onClick={onComplete}
                className="py-2 px-6 bg-blue-600 text-white rounded-full font-semibold hover:bg-blue-700 transition-colors"
            >
                Next Step
            </button>
          ) : (
            <button onClick={() => { setSubmitted(false); setSelected(null); }} className="text-sm underline hover:text-red-900">Try again</button>
          )}
        </motion.div>
      )}
    </div>
